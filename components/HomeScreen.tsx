...
const Pruduct = (props: {
  ...
  return (
    <>
      <TouchableOpacity
        onPress={() => props.navigation.navigate(props.route, {id: props.id})}>
        <View
          style={{
            display: 'flex',
            marginLeft: 15,
          ...
          <Image
            source={{
              uri: `https://www.api.octoberecommerce.duckdns.org/${props.cover_image}`,
            }}
            alt="sxeo"
            style={{
              resizeMode: 'contain',
          ...
          <View style={{display: 'flex', flexDirection: 'column'}}>
            <Text
              style={{fontFamily: 'GothamPro', width: 175, textAlign: 'left'}}
              fontSize="md">
              {props.name}
            </Text>
            <Text
            ...
            <Text
              style={{marginRight: 30, fontFamily: 'GothamPro'}}
              fontSize="md">
              {props.price}MMK
            </Text>
          </View>
          ...
          <View style={{position: 'absolute', bottom: 0, right: 0}}>
            <TouchableOpacity
              ...
              <MaterialCommunityIcons
                name={selected == 1 ? 'heart' : 'heart-outline'}
...
const PruductContainer = (props: {
  ...
  React.useEffect(() => {
    ...
    fetch(`https://api.october.com.mm/api/products/`)
      ...
      .then(data => {
        setNewarrvals(data.data.products);
        setErrorlog(true);
      })
      .catch(() => setErrorlog(false));
  ...
  if (props.type == 'catagory') {
    const filteredCatagory = newarrivals.filter(
      item => item.category[0].name == 'Body',
    );
...
const CatagoryContainer = (props: {
  ...
  React.useEffect(() => {
    fetch(`https://api.october.com.mm/api/categories/`)
      .then(response => response.json())
      .then(data => {
        setCatagory(data.data.categories);
        setRealcatagory(catagory.filter(item => item.id == 1));
        setErrorlog(true);
      })
      .catch(() => setErrorlog(false));
  ...
  return (
    <>
      <View style={styles.square}>
        <CatagoryBanner
          catagory_name={realcatagory[0]?.name}
          img_url={realcatagory[0]?.image}
        />
        <CPruductContainer
          name={props.name}
          navigation={props.navigation}
